...
daily_reports_data = [
  ...
  # 関連検索テスト用の追加日報（共通キーワード含む）
  {
    user: created_users[0],
    date: 3.days.ago.to_date,
    work_content: "TypeScriptを活用したReactアプリケーションのリファクタリングを実施しました。型安全性の向上とコードの可読性改善に取り組みました。",
    learned_points: "TypeScriptの型システムについて学習できました。インターフェースの活用方法や、ジェネリクスを使った型安全なコードの書き方を習得しました。開発時のエラー検出の改善効果を実感できました。",
    improvements: "テストケースの充実が必要だと感じました。また、型定義ファイルの管理方法を見直したいと思います。",
    status: "public"
  },
  
  {
    user: created_users[1],
    date: 2.days.ago.to_date,
    work_content: "データベースのインデックス戦略の見直しを行いました。複合インデックスの最適化とクエリパフォーマンスの改善に取り組みました。",
    learned_points: "インデックス設計の重要性を改めて学習しました。適切なインデックスがクエリパフォーマンスに与える影響の大きさを実感しました。EXPLAIN文による実行計画の分析方法も習得できました。",
    improvements: "定期的なパフォーマンス監視の仕組みを構築したいです。また、スロークエリの検出を自動化したいと思います。",
    status: "public"
  }
]
